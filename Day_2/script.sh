...
# Copy setup.sh to remote instance
echo "Copying setup.sh to remote instance..."
scp -i "$KEY_NAME.pem" -o StrictHostKeyChecking=no setup.sh ubuntu@$PUBLIC_IP:/home/ubuntu/

# Run setup.sh on the remote instance
echo "Running setup.sh remotely..."
ssh -i "$KEY_NAME.pem" -o StrictHostKeyChecking=no ubuntu@$PUBLIC_IP 'chmod +x setup.sh && sudo ./setup.sh'
